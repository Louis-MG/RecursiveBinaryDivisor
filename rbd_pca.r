...
getplot <- function(dir1) {
	...
	df = read.table(paste(parent_dir, '/', list.files(parent_dir, pattern = '.pca$')[1], sep = ''), sep = '\t')
	dfone = data.frame(names_parent, df[,1:3], stringsAsFactors = FALSE)
	df = data.frame(dfone, groups, stringsAsFactors = FALSE)
...
invisible(lapply(directories, getplot))
