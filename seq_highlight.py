...
with open(args.seq, 'r') as f:
	lines = f.readlines() #reads the sequences from Christophe
...
with open(args.fasta, 'r') as f:
	...
	for i in range(len(temp)) :
		if temp[i].startswith('>') : #creates a list of length twice the number of sequences
			seq_bank.append(temp[i]) #adds the id of the seq
			seq_bank.append(temp[i+1].rstrip()+temp[i+2].rstrip()+temp[i+3]) #adds the sequences

seq = []
for i in range(len(lines)):
	for j in range(len(seq_bank)):
		if lines[i] == seq_bank[j] :
			seq.append(seq_bank[j-1]) #contains the sequence references of the sequences
